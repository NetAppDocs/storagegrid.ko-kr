...
. * StorageGRID 설치* 열에서 소프트웨어 기반 노드 유형(RHEL, Ubuntu 또는 Debian, VMware)에 맞는 .tgz 또는 .zip 설치 아카이브를 선택합니다.
+

NOTE: 서비스 랩톱에서 Windows를 실행하는 경우 이 `.zip` 파일을 사용합니다.

. 설치 아카이브를 저장합니다.
. [[download-verification-package]]Linux 노드에서는 코드 서명 검증이 수동으로 수행됩니다.  선택적으로 설치 아카이브를 확인해야 하는 경우:
+
...
.. 다음 단계를 따르세요link:download-files-verify.html["설치 파일을 수동으로 확인합니다"] .
