...
summary: Swift 클라이언트가 헤드 작업을 실행하면 계정, 컨테이너 또는 객체가 있는지 확인하고 관련 메타데이터를 검색하도록 요청이 이루어집니다. 이 메시지는 작업이 성공하면 서버에서 발행합니다. 
---
= WHEA: 스위프트 헤드
:allow-uri-read: 
:icons: font
...
[role="lead"]
Swift 클라이언트가 헤드 작업을 실행하면 계정, 컨테이너 또는 객체가 있는지 확인하고 관련 메타데이터를 검색하도록 요청이 이루어집니다. 이 메시지는 작업이 성공하면 서버에서 발행합니다.
