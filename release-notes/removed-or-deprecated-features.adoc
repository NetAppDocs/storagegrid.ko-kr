...
sidebar: sidebar 
keywords:  
summary: StorageGRID 12.0에서 제거되거나 더 이상 사용되지 않는 기능에 대해 알아보세요. 업그레이드하기 전에 제거되거나 더 이상 사용되지 않는 기능이 클라이언트 애플리케이션이나 그리드 구성에 영향을 미치는지 확인하십시오. 
---
= StorageGRID에서 제거되거나 더 이상 사용되지 않는 기능
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
StorageGRID 12.0에서 제거되거나 더 이상 사용되지 않는 기능에 대해 알아보세요. 업그레이드하기 전에 제거되거나 더 이상 사용되지 않는 기능이 클라이언트 애플리케이션이나 그리드 구성에 영향을 미치는지 확인하십시오.
...
12.1
 a| 
https://docs.netapp.com/us-en/storagegrid-118/monitor/managing-alarms.html#configure-notifications-for-alarms-legacy-system["알람에 대한 알림 구성(레거시 시스템)(StorageGRID 11.8)"^]
