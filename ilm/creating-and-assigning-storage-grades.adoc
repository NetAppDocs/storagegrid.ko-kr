...
. 사용자 정의 저장 평점 정의:
+
.. 추가하려는 각 사용자 지정 저장 등급에 대해 *삽입* image:icon_nms_insert.gif["삽입 아이콘"]을 선택하여 행을 추가하십시오.
.. 설명 라벨을 입력합니다.
+
image:editing_storage_grades.gif["보관 등급 페이지"]

.. Apply Changes * 를 선택합니다.
.. 저장된 레이블을 수정해야 하는 경우, 선택적으로 *편집* image:icon_nms_edit.gif["편집 아이콘"]을 선택한 다음 *변경 사항 적용*을 선택하십시오.
+
...
+
.. LDR 목록에서 스토리지 노드를 찾고 *편집* 아이콘 image:icon_nms_edit.gif["편집 아이콘"]을 선택합니다.
.. 목록에서 적절한 스토리지 등급을 선택합니다.
+
image:assigning_storage_grades_to_storage_nodes.gif["보관 등급 페이지"]
