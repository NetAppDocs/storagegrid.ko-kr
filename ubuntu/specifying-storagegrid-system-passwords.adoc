...
NOTE: (`sgws-recovery-package-id-revision.zip`요약 페이지에서 * 설치 * 를 클릭하면 복구 패키지 파일을 다운로드하라는 메시지가 표시됩니다. link:../maintain/downloading-recovery-package.html["이 파일을 다운로드합니다"]설치를 완료해야 합니다. 시스템에 액세스하는 데 필요한 암호는 `Passwords.txt` 복구 패키지 파일에 포함된 파일에 저장됩니다.
