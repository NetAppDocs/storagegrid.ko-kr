...
+
이 예에서 한 진단에는 주의 상태가 있고 세 개의 진단에는 정상 상태가 있습니다.
...
** * 상태 값 *: StorageGRID 시스템 전체의 진단 상태 및 값을 보여 주는 그래프 및 표(스크린샷에는 표시되지 않는 표) 이 예에서는 StorageGRID 시스템의 모든 노드에 대한 현재 CPU 활용률이 표시됩니다. 모든 노드 값이 주의 및 주의 임계값 미만이므로 진단의 전체 상태는 정상입니다.
+
image::../media/support_diagnostics_cpu_utilization.png[진단 상태 값의 예]



. * 선택 사항 *: 이 진단과 관련된 Grafana 차트를 보려면 * Grafana 대시보드 * 를 선택하십시오.
+
이 링크는 일부 진단 유틸리티에는 표시되지 않습니다.

+
관련 Grafana 대시보드가 나타납니다. 이 예에서는 이 노드의 시간별 CPU 활용률 및 노드의 다른 Grafana 차트를 보여 주는 노드 대시보드가 표시됩니다.
