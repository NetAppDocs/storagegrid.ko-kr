...
* 당신은 가지고있다link:../admin/admin-group-permissions.html["관리자 권한"] PingFederate 서버용.
...
. 선택하세요<<password-validator,암호 자격 증명 유효성 검사기가 있습니다>> 당신이 창조했습니다.
...
. 관리 노드의 SSO 구성 페이지에서 다운로드한 SAML 메타데이터 파일을 선택합니다.
...
. 선택하세요<<signing-certificate,서명 인증서>> 귀하가 만들거나 가져왔습니다.
