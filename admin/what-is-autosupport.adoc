...
AutoSupport를 사용하면 문제를 더 빠르게 해결할 수 있습니다. 기술 지원 부서에서는 시스템의 스토리지 요구 사항을 모니터링하고 새 노드나 사이트를 추가해야 하는지 여부를 결정하는 데 도움을 줄 수 있습니다. 선택적으로 AutoSupport은 패키지를 하나의 추가 대상으로 보낼 수 있습니다.
...
NOTE: StorageGRID AutoSupport가 활성화되어 있지 않으면 그리드 관리자 대시보드에 메시지가 나타납니다. 이 메시지에는 AutoSupport 구성 페이지에 대한 링크가 포함되어 있습니다. 메시지를 닫으면 AutoSupport를 사용하지 않도록 설정해도 브라우저 캐시가 지워질 때까지 메시지가 다시 나타나지 않습니다.
