...
image:notification_multiple_sites.gif["사이트 1의 개체에 대한 작업이 사이트 1의 알림을 트리거하는 다이어그램을 나타냅니다"]

이후에 클라이언트가 데이터 센터 사이트 2에서 동일한 개체에 대해 S3 API 삭제 작업을 수행하면 삭제 작업에 대한 알림이 트리거되어 데이터 센터 사이트 2에서 전송됩니다.

image:notifications_site_2.gif["사이트 2의 개체에 대한 작업을 보여 주는 다이어그램이 사이트 2의 알림을 트리거합니다"]
