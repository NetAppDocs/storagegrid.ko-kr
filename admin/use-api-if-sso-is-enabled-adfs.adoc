...
[role="lead"]
만약 당신이 가지고 있다면link:../admin/how-sso-works.html["SSO(Single Sign-On) 구성 및 활성화"] SSO 공급자로 Active Directory를 사용하는 경우 Grid Management API 또는 Tenant Management API에 유효한 인증 토큰을 얻기 위해 일련의 API 요청을 발행해야 합니다.
...
+
image:sso_auth_python_script_adfs.png["AD FS에 대한 SSO 인증 스크립트"]
