...
summary: StorageGRID 테넌트 계정에 그리드 페더레이션 연결 사용 권한이 있는 경우 테넌트 관리 API를 사용하여 S3 액세스 키를 수동으로 복제할 수 있습니다. 
---
...
+
image:grid-federation-post-current-user-replicate.png["고유한 액세스 키를 복제하기 위한 사후 끝점"]
...
+
image:grid-federation-clone-access-key.png["테넌트 관리자 API 클론 액세스 키에 값을 입력합니다"]
...
+
image:grid-federation-post-other-user.png["다른 사용자의 액세스 키를 복제하기 위한 게시 끝점"]
