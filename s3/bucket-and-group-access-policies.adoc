...
S3:PutObject
 a| 
PutObject, CopyObject 및 CreateMultipartUpload 요청의 요청 헤더에서 확장된 값과 `object-lock-mode` 비교합니다.
...
S3:PutObjectRetention
 a| 
PutObjectRetention 요청의 XML 본문에서 확장된 것과 `object-lock-mode` 비교합니다.
...
S3:PutObject
 a| 
PutObject, CopyObject, CreateMultipartUpload, UploadPart, UploadPartCopy 및 CompleteMultipartUpload 요청의 요청 헤더에서 확장된 또는 `copy-source-sse-customer-algorithm` 와 `sse-customer-algorithm` 비교합니다.
