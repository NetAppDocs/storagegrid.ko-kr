...
StorageGRID 시스템에 대해 ID 페더레이션 또는 SSO(Single Sign-On)를 사용하려는 경우 이러한 기능을 활성화했습니다. 또한 ONTAP에서 사용할 테넌트 계정에 대한 루트 액세스 권한이 있어야 하는 통합 그룹도 알고 있습니다. link:../admin/using-identity-federation.html["ID 페더레이션을 사용합니다"]및 을 link:../admin/how-sso-works.html["Single Sign-On 구성"]참조하십시오.
