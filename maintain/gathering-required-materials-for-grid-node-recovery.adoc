...
---
= StorageGRID 그리드 노드 복구에 필요한 자료를 모으십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
내용 `.zip` 시스템이 수정될 때마다 파일이 업데이트됩니다.  이러한 변경 사항을 적용한 후에는 최신 버전의 복구 패키지를 안전한 장소에 보관하라는 지시를 받게 됩니다.  최신 사본을 사용하여 그리드 장애를 복구합니다.

모든 관리 노드에 로그인한 후 Grid Manager를 사용하여 복구 패키지를 다운로드하세요.  *유지관리* > *시스템* > *복구 패키지*를 선택하세요.

Grid Manager에 액세스할 수 없는 경우 ADC 서비스가 포함된 일부 스토리지 노드에서 복구 패키지의 암호화된 사본을 찾을 수 있습니다.  각 스토리지 노드에서 복구 패키지의 다음 위치를 조사하세요. `/var/local/install/sgws-recovery-package-_grid-id_-_revision_.zip.gpg` 가장 높은 개정 번호의 복구 패키지를 사용하세요.
...
. 플랫폼과 복구해야 할 그리드 노드를 기반으로 필요한 파일을 선택하려면 해당 플랫폼에 맞는 섹션으로 건너뛰세요.
+
